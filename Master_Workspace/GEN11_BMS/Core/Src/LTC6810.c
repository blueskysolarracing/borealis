...
void readVolt(uint16_t* voltArray, SPI_HandleTypeDef *spi, UART_HandleTypeDef* uart){
	...
	LTC6810CommandGenerate(VmessageInBinary);//generate the "check voltage command"
	HAL_GPIO_WritePin(LTC6810_CS_GPIO_Port, LTC6810_CS_Pin, GPIO_PIN_RESET); //slave select low, transmit begin
	HAL_Delay(1); //require several us
	HAL_SPI_Transmit(spi, dataToSend, 4/*byte*/, 100);

	//now receive those data
	uint8_t dataToReceive[6];
	HAL_Delay(1); //ADD DELAY between Transmit & Receive
	HAL_SPI_Receive(spi, dataToReceive, 8, 100);
	HAL_GPIO_WritePin(LTC6810_CS_GPIO_Port, LTC6810_CS_Pin, GPIO_PIN_SET);
	//Maybe Try fix this BUG???????

	VmessageInBinary = 0b100;  //read cell voltage reg group 1;
	LTC6810CommandGenerate(VmessageInBinary);
	HAL_GPIO_WritePin(LTC6810_CS_GPIO_Port, LTC6810_CS_Pin, GPIO_PIN_RESET); //slave select low, transmit begin
	HAL_Delay(1); //require several us
	HAL_SPI_Transmit(spi, dataToSend, 4/*byte*/, 100);
	HAL_Delay(1); //ADD DELAY between Transmit & Receive
	...
	voltArray[0] = dataToReceive[1] * 256 + dataToReceive[0];
	voltArray[1] = dataToReceive[3] * 256 + dataToReceive[2];
	voltArray[2] = dataToReceive[5] * 256 + dataToReceive[4];
...
//	voltArray[2] = voltageDataConversion(dataToReceive[4], dataToReceive[5]) / 10000.0;
	...
	voltArray[3] = dataToReceive[1] * 256 + dataToReceive[0];
	voltArray[4] = dataToReceive[3] * 256 + dataToReceive[2];
	voltArray[5] = dataToReceive[5] * 256 + dataToReceive[4];
...
//	voltArray[5] = voltageDataConversion(dataToReceive[4], dataToReceive[5]) / 10000.0;
	...
	//Transmit through UART for debug
	if (DEBUG_EN){
		char MSG[100];
		sprintf(MSG, "Voltage #1: %fV\nVoltage #2: %fV\nVoltage #3: %fV\nVoltage #4: %fV\nVoltage #5: %fV\nVoltage #6: %fV\n\n\n",
				voltArray[0], voltArray[1], voltArray[2], voltArray[3], voltArray[4], voltArray[5]);
		HAL_UART_Transmit(uart, (uint8_t *) MSG, sizeof(MSG), 100);
	}
}
...
void readTemp(uint16_t* tempArray, int DCC5, int DCC4, int DCC3, int DCC2, int DCC1, SPI_HandleTypeDef *spi, UART_HandleTypeDef* uart){
	...
	while(cycle<3){
		if(cycle ==0){
			LTC6810Init(spi, 0, 0, 0, DCC5, DCC4, DCC3, DCC2, DCC1);}//channel 1
		else if(cycle == 1){
			LTC6810Init(spi, 0, 0, 1, DCC5, DCC4, DCC3, DCC2, DCC1);}//channel 2
		else if(cycle == 2){
			LTC6810Init(spi, 0, 1, 0, DCC5, DCC4, DCC3, DCC2, DCC1);}//channel 3
	//cycle are 0,1,2
		messageInBinary = 0b10100010010;  //conversion GPIO1, command AXOW
		LTC6810CommandGenerate(messageInBinary);
		HAL_Delay(1);
		...
		HAL_SPI_Receive(spi, dataToReceive, 6, 100);
		HAL_GPIO_WritePin(GPIOC, GPIO_PIN_9, GPIO_PIN_SET);
		//write value into array
		tempArray[cycle] = dataToReceive[3] * 256 + dataToReceive[2];
		//(float)dataToReceive[2];//msb of data
		//temp2 = dataToReceive[3];
		//sprintf(MSG, "voltage:  %d,%d, at%d \r\n ",temp1,temp2,cycle );
		cycle++;
	...
	if (DEBUG_EN){
		char msg[50];
		for (int i = 0; i < 3; i++){
			sprintf(msg, "Temperature #%d: %fC\n", i, tempArray[i]);
			HAL_UART_Transmit(uart, (uint8_t *) msg, sizeof(msg), 10);
		}
	}
}
...
int LTC6810Init(SPI_HandleTypeDef *spi, int GPIO4, int GPIO3, int GPIO2 ,int DCC5, int DCC4, int DCC3, int DCC2, int DCC1) {
	...
	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_9, GPIO_PIN_RESET); //slave select low, transmit begin
	HAL_Delay(1); //require several us
	HAL_SPI_Transmit(spi, dataToSend, 12, 100);
	HAL_Delay(1);
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_11, GPIO_PIN_SET);
	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_9, GPIO_PIN_SET);
